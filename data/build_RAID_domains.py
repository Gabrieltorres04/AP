...
save_path = os.path.join(script_dir, "raw", "RAID_domains_filtered.csv")

# Academic/scientific domains only
KEEP_DOMAINS = {"abstracts", "wiki"}
...
if os.path.exists(save_path):
    ...
    print(f"Streaming RAID, collecting: {KEEP_MODELS} (no attacks, domains: {KEEP_DOMAINS})...")
    rows = []
    dataset = load_dataset("liamdugan/raid", "raid", streaming=True)
    for row in dataset["train"]:
        if row["model"] in KEEP_MODELS and row["attack"] == "none" and row["domain"] in KEEP_DOMAINS:
...
out_path = os.path.join(script_dir, "domain_treated_datasets", "dataset_RAID_domains.csv")
